...
if __name__ == "__main__":
    ...
    r = weibo.login("", "")
    if r:
        with open('weibo_cookie', 'wb') as f:
            pickle.dump(requests.utils.dict_from_cookiejar(weibo.session.cookies), f)
            # with open('weibo_cookie', 'rb') as f:
            #     cookies = requests.utils.cookiejar_from_dict(pickle.load(f))
            #     requests.session().cookies = cookies
